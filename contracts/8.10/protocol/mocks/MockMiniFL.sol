...
contract MockMiniFL is OwnableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  struct PoolInfo {
    uint128 accMolePerShare;
    uint64 lastRewardTime;
    uint64 allocPoint;
    bool isDebtTokenPool;
  }

  IERC20Upgradeable public MOLE;
  ...
  uint256 public molePerSecond;
  uint256 private constant ACC_MOLE_PRECISION = 1e12;
  uint256 public maxMolePerSecond;
  ...
  event LogUpdatePool(uint256 indexed pid, uint64 lastRewardTime, uint256 stakedBalance, uint256 accMolePerShare);
  event LogMolePerSecond(uint256 molePerSecond);
  event LogApproveStakeDebtToken(uint256 indexed _pid, address indexed _staker, bool allow);
  event LogSetMaxMolePerSecond(uint256 maxMolePerSecond);

  /// @param _mole The MOLE token contract address.
  function initialize(address _mole, uint256 _maxMolePerSecond) external initializer {
    OwnableUpgradeable.__Ownable_init();
    ReentrancyGuardUpgradeable.__ReentrancyGuard_init();

    MOLE = IERC20Upgradeable(_mole);
    maxMolePerSecond = _maxMolePerSecond;
  }
  ...
  function addPool(
    ...
    if (address(_stakingToken) == address(MOLE)) revert MiniFL_InvalidArguments();
  ...
  /// @notice Update the given pool's MOLE allocation point and `IRewarder` contract.
  ...
  /// @notice Sets the MOLE per second to be distributed. Can only be called by the owner.
  /// @param _molePerSecond The amount of MOLE to be distributed per second.
  /// @param _withUpdate If true, do mass update pools
  function setMolePerSecond(uint256 _molePerSecond, bool _withUpdate) external onlyOwner {
    if (_molePerSecond > maxMolePerSecond) revert MiniFL_InvalidArguments();

    if (_withUpdate) massUpdatePools();
    molePerSecond = _molePerSecond;
    emit LogMolePerSecond(_molePerSecond);
  }

  /// @notice View function to see pending MOLE on frontend.
  /// @param _pid The index of the pool. See `poolInfo`.
  /// @param _user Address of a user.
  /// @return pending MOLE reward for a given user.
  function pendingMole(uint256 _pid, address _user) external view returns (uint256) {
    ...
    if (block.timestamp > pool.lastRewardTime && stakedBalance != 0) {
      ...
      uint256 moleReward = (timePast * molePerSecond * pool.allocPoint) / totalAllocPoint;
      accMolePerShare = accMolePerShare + ((moleReward * ACC_MOLE_PRECISION) / stakedBalance);
    }

    return (((user.amount * accMolePerShare) / ACC_MOLE_PRECISION).toInt256() - user.rewardDebt).toUint256();
  }
  ...
  function _updatePool(uint256 pid) internal returns (PoolInfo memory) {
    ...
    if (block.timestamp > pool.lastRewardTime) {
      ...
      if (stakedBalance > 0) {
        uint256 timePast = block.timestamp - pool.lastRewardTime;
        uint256 moleReward = (timePast * molePerSecond * pool.allocPoint) / totalAllocPoint;
        pool.accMolePerShare =
          pool.accMolePerShare +
          ((moleReward * ACC_MOLE_PRECISION) / stakedBalance).toUint128();
      }
      pool.lastRewardTime = block.timestamp.toUint64();
      poolInfo[pid] = pool;
      emit LogUpdatePool(pid, pool.lastRewardTime, stakedBalance, pool.accMolePerShare);
    }
    return pool;
  }
  ...
  /// @notice Deposit tokens to MiniFL for MOLE allocation.
  ...
  function deposit(
    ...
    user.rewardDebt = user.rewardDebt + ((_amount * pool.accMolePerShare) / ACC_MOLE_PRECISION).toInt256();
  ...
  /// @notice Harvest MOLE rewards
  function harvest(
    uint256 /*_pid*/
  ) external nonReentrant {
    MOLE.safeTransfer(msg.sender, 10 * 1e18);
  }
  ...
  /// @param _maxMolePerSecond The max reward per second
  function setMaxMolePerSecond(uint256 _maxMolePerSecond) external onlyOwner {
    if (_maxMolePerSecond <= molePerSecond) revert MiniFL_InvalidArguments();
    maxMolePerSecond = _maxMolePerSecond;
    emit LogSetMaxMolePerSecond(_maxMolePerSecond);
  }
}
