...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  */
  ...
  for (let i = 0; i < workerInfos.length; i++) {
    console.log("===================================================================================");
    console.log(`>> Deploying an upgradable TraderJoeV3Worker03 contract for ${workerInfos[i].WORKER_NAME}`);
    const TraderJoeV3Worker03 = (await ethers.getContractFactory("TraderJoeV3Worker03", deployer)) as TraderJoeV3Worker03__factory;
    const traderJoeV3Worker03 = (await upgrades.deployProxy(TraderJoeV3Worker03, [
    ...
    ],)) as TraderJoeV3Worker03;
    const deployedTx = await traderJoeV3Worker03.deployTransaction.wait(20);
    console.log(`>> Deployed at ${traderJoeV3Worker03.address}`);
    console.log(`>> Deployed block: ${deployedTx.blockNumber}`);

    console.log(">> Waiting for TraderJoeV3Worker03 to be ready...");
    ...
    await traderJoeV3Worker03.setReinvestorOk([workerInfos[i].REINVEST_BOT], true, { nonce: nonce++ });
    console.log("✅ Done");
    ...
    await traderJoeV3Worker03.setStrategyOk(okStrats, true, { nonce: nonce++ });
    console.log("✅ Done");
    ...
    if (workerInfos[i].PARTIAL_CLOSE_LIQ_STRAT_ADDR != "") {
      ...
      await partialCloseLiquidate.setWorkersOk([traderJoeV3Worker03.address], true, { nonce: nonce++ });
    }
    ...
    if (workerInfos[i].PARTIAL_CLOSE_MINIMIZE_STRAT_ADDR != "") {
      ...
      await partialCloseMinimize.setWorkersOk([traderJoeV3Worker03.address], true, { nonce: nonce++ });
    }
    console.log("✅ Done");

    if (workerInfos[i].BENEFICIAL_VAULT) {
      console.log(">> config baneficial vault");
      await traderJoeV3Worker03.setBeneficialVaultConfig(
    ...
    if ((await workerConfig.owner()).toLowerCase() === timelock.address.toLowerCase()) {
      ...
      console.log(
        `await timelock.executeTransaction('${workerInfos[i].WORKER_CONFIG_ADDR}', '0', 'setConfigs(address[],(bool,uint64,uint64,uint64)[])', ethers.utils.defaultAbiCoder.encode(['address[]','(bool acceptDebt,uint64 workFactor,uint64 killFactor,uint64 maxPriceDiff)[]'],[['${traderJoeV3Worker03.address}'], [{acceptDebt: true, workFactor: ${workerInfos[i].WORK_FACTOR}, killFactor: ${workerInfos[i].KILL_FACTOR}, maxPriceDiff: ${workerInfos[i].MAX_PRICE_DIFF}}]]), ${workerInfos[i].EXACT_ETA})`
      );
      console.log("✅ Done");
    } else {
    ...
    if ((await vaultConfig.owner()).toLowerCase() === timelock.address.toLowerCase()) {
      ...
      const setWorkersTx = await timelock.queueTransaction(
        ...
        ethers.utils.defaultAbiCoder.encode(
          ["address[]", "address[]"],
          [[traderJoeV3Worker03.address], [workerInfos[i].WORKER_CONFIG_ADDR]]
        ),
        workerInfos[i].EXACT_ETA,
        { nonce: nonce++ }
      );
      ...
      console.log("generate timelock.executeTransaction:");
      console.log(
        `await timelock.executeTransaction('${workerInfos[i].VAULT_CONFIG_ADDR}', '0','setWorkers(address[],address[])', ethers.utils.defaultAbiCoder.encode(['address[]','address[]'],[['${traderJoeV3Worker03.address}'], ['${workerInfos[i].WORKER_CONFIG_ADDR}']]), ${workerInfos[i].EXACT_ETA})`
      );
      console.log("✅ Done");
    } else {
      console.log(">> Linking VaultConfig with WorkerConfig");
      (
        await vaultConfig.setWorkers([traderJoeV3Worker03.address], [workerInfos[i].WORKER_CONFIG_ADDR], { nonce: nonce++ })
      ).wait(3);
      console.log("✅ Done");
...
export default func;
func.tags = ["TraderJoeV3Workers03"];
