...
contract Shield is Ownable {
  ...
  event SetMolePerBlock(uint256 indexed _molePerBlock);
  ...
  /// @dev Set MOLE per Block on FLV1. Effect immediately on the next block.
  /// @param _molePerBlock The new molePerBlock
  function setMolePerBlock(uint256 _molePerBlock) external onlyOwner {
    fairLaunchV1.setMolePerBlock(_molePerBlock);
    emit SetMolePerBlock(_molePerBlock);
  }
  ...
  /// @dev Maunally mint MOLE warchest portion.
