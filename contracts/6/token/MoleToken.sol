...
// MoleToken with Governance.
contract MoleToken is ERC20("MoleToken", "MOLE"), Ownable {
  uint256 private constant CAP = 80000000e18;
  uint256 private _totalLock;

  uint256 public startReleaseBlock;
  uint256 public endReleaseBlock;
  uint256 public constant MANUAL_MINT_LIMIT = 9600000e18;
  ...
  function canUnlockAmount(address _account) public view returns (uint256) {
    // When block number less than startReleaseBlock, no MOLEs can be unlocked
    if (block.number < startReleaseBlock) {
      return 0;
    }
    // When block number more than endReleaseBlock, all locked MOLEs can be unlocked
    else if (block.number >= endReleaseBlock) {
      return _locks[_account];
    }
    // When block number is more than startReleaseBlock but less than endReleaseBlock,
    // some MOLEs can be released
    else
    {
  ...
  function unlock() external {
    require(_locks[msg.sender] > 0, "no locked MOLEs");
  ...
  // @dev move MOLEs with its locked funds to another account
  ...
  function delegateBySig(
    ...
    require(signatory != address(0), "MOLE::delegateBySig: invalid signature");
    require(nonce == nonces[signatory]++, "MOLE::delegateBySig: invalid nonce");
    require(now <= expiry, "MOLE::delegateBySig: signature expired");
  ...
  function getPriorVotes(address account, uint256 blockNumber) external view returns (uint256) {
    require(blockNumber < block.number, "MOLE::getPriorVotes: not yet determined");
  ...
  function _delegate(address delegator, address delegatee) internal {
    ...
    uint256 delegatorBalance = balanceOf(delegator); // balance of underlying MOLEs (not scaled);
  ...
  function _writeCheckpoint(
    ...
    uint32 blockNumber = safe32(block.number, "MOLE::_writeCheckpoint: block number exceeds 32 bits");
