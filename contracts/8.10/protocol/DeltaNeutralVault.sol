...
contract DeltaNeutralVault is ERC20Upgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
  ...
  /// @param _moleToken Mole token address.
  ...
  function initialize(
    ...
    if (address(moleToken) == address(0)) revert DeltaNeutralVault_InvalidInitializedAddress();
  ...
  function reinvest(
    ...
    uint256 _moleBountyBps = config.moleBountyBps();
    uint256 _moleBeneficiaryBps = config.moleBeneficiaryBps();
    ...
    uint256 _moleBefore = IERC20Upgradeable(moleToken).balanceOf(address(this));
    ...
    uint256 _moleAfter = IERC20Upgradeable(moleToken).balanceOf(address(this));

    // 2. collect mole bounty & distribute to MOLE beneficiary
    uint256 _bounty = ((_moleBountyBps) * (_moleAfter - _moleBefore)) / MAX_BPS;
    uint256 _beneficiaryShare = (_bounty * _moleBeneficiaryBps) / MAX_BPS;
    if (_beneficiaryShare > 0)
      IERC20Upgradeable(moleToken).safeTransfer(config.moleBeneficiary(), _beneficiaryShare);
    IERC20Upgradeable(moleToken).safeTransfer(config.moleReinvestFeeTreasury(), _bounty - _beneficiaryShare);

    // 3. swap mole
    uint256 _rewardAmount = _moleAfter - _bounty;
    ISwapRouter _router = ISwapRouter(config.getSwapRouter());
    IERC20Upgradeable(moleToken).approve(address(_router), _rewardAmount);
