...
/// @title PangolinWorker03 is a worker with reinvest-optimized and beneficial vault buyback functionalities
contract PangolinWorker03 is OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe, IWorker03 {
  /// @notice Libraries
  ...
  function initialize(
    ...
    require(baseToken != png, "baseToken must !png");
    ...
    require(reinvestPath[0] == png && reinvestPath[reinvestPath.length - 1] == baseToken, "bad reinvest path");
  }
  ...
  function shareToBalance(uint256 share) public view returns (uint256) {
    ...
    (uint256 totalBalance, ) = miniChefV2.userInfo(pid, address(this));
  ...
  function _reinvest(
    ...
    // 1. Harvest all the rewards. Return if reward <= _reinvestThershold.
    miniChefV2.harvest(pid, address(this));
    uint256 reward = png.balanceOf(address(this));
    if (reward <= _reinvestThreshold) return;

    // 2. Approve tokens
    png.safeApprove(address(router), uint256(-1));
    address(lpToken).safeApprove(address(miniChefV2), uint256(-1));
    ...
    if (bounty > 0) {
      ...
      png.safeTransfer(_treasuryAccount, bounty.sub(beneficialVaultBounty));
    }
    ...
    miniChefV2.deposit(pid, lpToken.balanceOf(address(this)), address(this));

    // 7. Reset approvals
    png.safeApprove(address(router), 0);
    address(lpToken).safeApprove(address(miniChefV2), 0);
  ...
  /// @param _beneficialVaultBounty - The amount of PNG to be swapped to BTOKEN & send back to the Vault.
  /// @param _callerBalance - The balance that is owned by the msg.sender within the execution scope.
  ...
  function _addShare(uint256 id) internal {
    ...
    if (balance > 0) {
      // 1. Approve token to be spend by Pangolin's MasterChef
      address(lpToken).safeApprove(address(miniChefV2), uint256(-1));
      // 2. Convert balance to share
      uint256 share = balanceToShare(balance);
      require(share > 0, "no zero share");
      // 3. Deposit balance to Pangolin's MasterChef
      // and also force reward claim, to mimic the behaviour of Pangolin's MasterChef
      miniChefV2.deposit(pid, balance, address(this));
      // 4. Update shares
  ...
  function _removeShare(uint256 id) internal {
    ...
    if (share > 0) {
      ...
      miniChefV2.withdraw(pid, balance, address(this));
  ...
  function setReinvestConfig(
    ...
    require(_reinvestPath[0] == png && _reinvestPath[_reinvestPath.length - 1] == baseToken, "bad _reinvestPath");
  ...
  function setRewardPath(address[] calldata _rewardPath) external onlyOwner {
    ...
    require(_rewardPath[0] == png && _rewardPath[_rewardPath.length - 1] == beneficialVault.token(), "bad _rewardPath");
  ...
  function setBeneficialVaultConfig(
    ...
    require(
      _rewardPath[0] == png && _rewardPath[_rewardPath.length - 1] == _beneficialVault.token(),
      "bad _rewardPath"
    );
