...
/// @title TraderJoeBoostedStakingWorker03 is a worker with reinvest-optimized and beneficial vault buyback functionalities
contract TraderJoeBoostedStakingWorker03 is OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe, IWorker03 {
  /// @notice Libraries
  ...
  function initialize(
    ...
    pid = _pid;
    (IERC20 _lpToken, , , , , , , , ) = boostedMasterChefJoe.poolInfo(_pid);
  ...
  function _reinvest(
    ...
    if(reward > stakeJoeAmount){
      reward = reward.sub(stakeJoeAmount);
    }else{
      reward = 0;
    }
    ...
    if (bounty > 0) {
      ...
      //stake
      uint256 stakeJoeBounty = bounty.sub(beneficialVaultBounty).mul(stakeJoeBountyBps) / 10000;
      if (stakeJoeBounty > 0) _rewardToStakeJoe(stakeJoeBounty);
      joe.safeTransfer(_treasuryAccount, bounty.sub(beneficialVaultBounty).sub(stakeJoeBounty));
    }
  ...
  function _rewardToStakeJoe(uint256 _stakeJoeBounty) internal {
    stakeJoeAmount = stakeJoeAmount.add(_stakeJoeBounty);
