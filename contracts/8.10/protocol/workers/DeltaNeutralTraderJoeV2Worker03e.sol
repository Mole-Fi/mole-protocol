...
/// @title DeltaNeutralTraderJoeV2Worker03e(expended) is a TraderJoeV2Worker03 worker with reinvest-optimized and beneficial vault buyback functionalities
contract DeltaNeutralTraderJoeV2Worker03e is OwnableUpgradeable, ReentrancyGuardUpgradeable, IWorker03 {
  /// @notice Libraries
  ...
  function initialize(
    ...
    (ERC20Upgradeable _lpToken, , , , IRewarder _rewarder) = masterChefJoeV2.poolInfo(_pid);
  ...
  function _masterChefJoeV2Deposit() internal {
    uint256 balance = lpToken.balanceOf(address(this));
    if (balance > 0) {
      address(lpToken).safeApprove(address(masterChefJoeV2), type(uint256).max);
  ...
  function _masterChefJoeV2Withdraw() internal {
    uint256 _totalLpBalance = totalLpBalance;
    masterChefJoeV2.withdraw(pid, _totalLpBalance);
    totalLpBalance = 0;
