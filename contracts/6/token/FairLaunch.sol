...
contract FairLaunch is IFairLaunch, Ownable, ReentrancyGuard {
  ...
  struct UserInfo {
    ...
    //
    // We do some fancy math here. Basically, any point in time, the amount of Moles
    // entitled to a user but is pending to be distributed is:
    //
    //   pending reward = (user.amount * pool.accMolePerShare) - user.rewardDebt
    //
    // Whenever a user deposits or withdraws Staking tokens to a pool. Here's what happens:
    //   1. The pool's `accMolePerShare` (and `lastRewardBlock`) gets updated.
  ...
  // The Mole TOKEN!
  MoleToken public mole;
  // Dev address.
  address public devaddr;
  // Percentage of pool rewards that goto the devs.For example 10% format is 1000
  uint256 public devPercent;
  // MOLE tokens created per block.
  uint256 public molePerBlock;
  // Bonus muliplier for early mole makers.
  uint256 public bonusMultiplier;
  // Block number when bonus MOLE period ends.
  ...
  // The block number when MOLE mining starts.
  ...
  */
  ...
  function setMolePerBlock(uint256 _molePerBlock) external onlyOwner {
    molePerBlock = _molePerBlock;
  }
  ...
  // Update the given pool's MOLE allocation point. Can only be called by the owner.
  function setPool(
    uint256 _pid,
    uint256 _allocPoint,
    bool /* _withUpdate */
  ...
  */
  ...
  // View function to see pending MOLEs on frontend.
  function pendingMole(uint256 _pid, address _user) external view override returns (uint256) {
    ...
    if (block.number > pool.lastRewardBlock && lpSupply != 0) {
      ...
      uint256 moleReward = multiplier.mul(molePerBlock).mul(pool.allocPoint).div(totalAllocPoint);
      accMolePerShare = accMolePerShare.add(moleReward.mul(1e12).div(lpSupply));
    }
    return user.amount.mul(accMolePerShare).div(1e12).sub(user.rewardDebt);
  }
  ...
  function updatePool(uint256 _pid) public override {
    ...
    uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
    uint256 moleReward = multiplier.mul(molePerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    mole.mint(devaddr, moleReward.mul(devPercent).div(10000));
    mole.mint(address(this), moleReward);
    pool.accMolePerShare = pool.accMolePerShare.add(moleReward.mul(1e12).div(lpSupply));
    // update accMolePerShareTilBonusEnd
    if (block.number <= bonusEndBlock) {
      mole.lock(devaddr, moleReward.mul(bonusLockUpBps).div(100000));
      pool.accMolePerShareTilBonusEnd = pool.accMolePerShare;
    }
    if (block.number > bonusEndBlock && pool.lastRewardBlock < bonusEndBlock) {
      uint256 moleBonusPortion = bonusEndBlock
        .sub(pool.lastRewardBlock)
        .mul(bonusMultiplier)
        .mul(molePerBlock)
        .mul(pool.allocPoint)
        .div(totalAllocPoint);
      mole.lock(devaddr, moleBonusPortion.mul(bonusLockUpBps).div(100000));
      pool.accMolePerShareTilBonusEnd = pool.accMolePerShareTilBonusEnd.add(
        moleBonusPortion.mul(1e12).div(lpSupply)
      );
    }
    pool.lastRewardBlock = block.number;
  }
  ...
  function deposit(
    ...
    user.bonusDebt = user.amount.mul(pool.accMolePerShareTilBonusEnd).div(1e12);
  ...
  function _harvest(address _to, uint256 _pid) internal {
    ...
    require(pending <= mole.balanceOf(address(this)), "wtf not enough mole");
    uint256 bonus = user.amount.mul(pool.accMolePerShareTilBonusEnd).div(1e12).sub(user.bonusDebt);
    safeMoleTransfer(_to, pending);
    mole.lock(_to, bonus.mul(bonusLockUpBps).div(10000));
  }
  ...
  // Safe mole transfer function, just in case if rounding error causes pool to not have enough MOLEs.
  function safeMoleTransfer(address _to, uint256 _amount) internal {
    uint256 moleBal = mole.balanceOf(address(this));
    if (_amount > moleBal) {
      require(mole.transfer(_to, moleBal), "failed to transfer MOLE");
    } else {
      require(mole.transfer(_to, _amount), "failed to transfer MOLE");
    }
  }
}
