...
import {
  ...
  TraderJoeV3Worker03e,
  TraderJoeV3Worker03e__factory,
  Timelock__factory,
  ...
  */
  ...
  const shortWorkerInfos: ITraderJoeSwapWorkerInput[] = [
    {
      ...
      REINVEST_THRESHOLD: ethers.utils.parseUnits('1', 18).toString(),
      REWARDER_REINVEST_PATH: ["QI", "WAVAX"],
      REWARDER_REINVEST_THRESHOLD: ethers.utils.parseUnits('1', 18).toString(),
      WORK_FACTOR: "5200",
      KILL_FACTOR: "7000",
  ...
  const workerInfos: ITraderJoeWorkerInfo[] = shortWorkerInfos.map((n) => {
    ...
    return {
      ...
      MASTER_CHEF_ADDR: config.Exchanges.TraderJoe!.MasterChefJoeV3,
  ...
  for (let i = 0; i < workerInfos.length; i++) {
    console.log("===================================================================================");
    console.log(`>> Deploying an upgradable TraderJoeV3Worker03e contract for ${workerInfos[i].WORKER_NAME}`);
    const TraderJoeV3Worker03e = (await ethers.getContractFactory("TraderJoeV3Worker03e", deployer)) as TraderJoeV3Worker03e__factory;
    const traderJoeV3Worker03e = (await upgrades.deployProxy(TraderJoeV3Worker03e, [
    ...
    ])) as TraderJoeV3Worker03e;
    const deployedTx = await traderJoeV3Worker03e.deployTransaction.wait(20);
    console.log(`>> Deployed at ${traderJoeV3Worker03e.address}`);
    console.log(`>> Deployed block: ${deployedTx.blockNumber}`);

    console.log(">> Waiting for TraderJoeV3Worker03e to be ready...");
    ...
    await traderJoeV3Worker03e.setReinvestorOk([workerInfos[i].REINVEST_BOT], true, { nonce: nonce++ });
    console.log("✅ Done");
    ...
    await traderJoeV3Worker03e.setStrategyOk(okStrats, true, { nonce: nonce++ });
    console.log("✅ Done");
    ...
    if (workerInfos[i].PARTIAL_CLOSE_LIQ_STRAT_ADDR != "") {
      ...
      await partialCloseLiquidate.setWorkersOk([traderJoeV3Worker03e.address], true, { nonce: nonce++ });
    }
    ...
    if (workerInfos[i].PARTIAL_CLOSE_MINIMIZE_STRAT_ADDR != "") {
      ...
      await partialCloseMinimize.setWorkersOk([traderJoeV3Worker03e.address], true, { nonce: nonce++ });
    }
    console.log("✅ Done");

    if (workerInfos[i].BENEFICIAL_VAULT) {
      console.log(">> config baneficial vault");
      await traderJoeV3Worker03e.setBeneficialVaultConfig(
    ...
    if (workerInfos[i].REWARDER_BENEFICIAL_VAULT) {
      ...
      await traderJoeV3Worker03e.setRewarderBeneficialVaultConfig(
    ...
    if ((await vaultConfig.owner()).toLowerCase() === timelock.address.toLowerCase()) {
      ...
      const setWorkersTx = await timelock.queueTransaction(
        ...
        ethers.utils.defaultAbiCoder.encode(
          ["address[]", "address[]"],
          [[traderJoeV3Worker03e.address], [workerInfos[i].WORKER_CONFIG_ADDR]]
        ),
        workerInfos[i].EXACT_ETA,
        { nonce: nonce++ }
      );
      ...
      console.log("generate timelock.executeTransaction:");
      console.log(
        `await timelock.executeTransaction('${workerInfos[i].VAULT_CONFIG_ADDR}', '0','setWorkers(address[],address[])', ethers.utils.defaultAbiCoder.encode(['address[]','address[]'],[['${traderJoeV3Worker03e.address}'], ['${workerInfos[i].WORKER_CONFIG_ADDR}']]), ${workerInfos[i].EXACT_ETA})`
      );
      console.log("✅ Done");
    } else {
      console.log(">> Linking VaultConfig with WorkerConfig");
      (
        await vaultConfig.setWorkers([traderJoeV3Worker03e.address], [workerInfos[i].WORKER_CONFIG_ADDR], { nonce: nonce++ })
      ).wait(3);
      console.log("✅ Done");
...
export default func;
func.tags = ["TraderJoeV3Workers03e"];
