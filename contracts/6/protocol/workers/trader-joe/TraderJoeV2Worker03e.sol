...
/// @title TraderJoeV2Worker03e(expended) is a worker with reinvest-optimized and beneficial vault buyback functionalities
contract TraderJoeV2Worker03e is OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe, IWorker03 {
  /// @notice Libraries
  ...
  event Reinvest(
    ...
    uint256 reward,
    uint256 bounty,
    uint256 rewarderReward,
    uint256 rewarderBounty
  );
  ...
  function initialize(
    ...
    address[] calldata _rewarderReinvestPath,
    ...
    rewarderReinvestThreshold = _rewarderReinvestThreshold;
    rewarderReinvestPath = _rewarderReinvestPath;
    rewarderTreasuryAccount = _treasuryAccount;
    rewarderTreasuryBountyBps = _reinvestBountyBps;
    rewarderMaxReinvestBountyBps = 900;
    ...
    require(baseToken != rewarderRewardToken, "baseToken must !rewordToken");
    require(rewarderRewardToken != joe, "rewardToken must !joe");
    require(rewarderRewardToken != address(lpToken), "rewardToken must !lpToken");
    ...
    require(rewarderReinvestPath.length >= 2, "_rewarderReinvestPath must >= 2");
    require(
      rewarderReinvestPath[0] == rewarderRewardToken &&
        rewarderReinvestPath[rewarderReinvestPath.length - 1] == baseToken,
      "bad rewarder reinvest path"
    );
  }
  ...
  /// @param _rewarderTreasuryBountyBps - The rewarder bounty bps deducted from the reinvest reward.
  ...
  /// @param _rewarderReinvestThreshold - The threshold to be reinvested rewarder reward if reward pass over.
  function _reinvest(
    address _treasuryAccount,
    ...
    masterChefJoeV2.withdraw(pid, 0);
    uint256 reward = joe.balanceOf(address(this));
    uint256 rewarderReward = rewarderRewardToken.balanceOf(address(this));
    ...
    uint256 bounty = 0;
    if (reinvestReward) {
      bounty = reward.mul(_treasuryBountyBps) / 10000;
      if (bounty > 0) {
    ...
    if (reinvestRewarderReward) {
      rewarderBounty = rewarderReward.mul(_rewarderTreasuryBountyBps) / 10000;
      if (rewarderBounty > 0) {
        uint256 beneficialVaultBounty = rewarderBounty.mul(beneficialVaultBountyBps) / 10000;
        if (beneficialVaultBounty > 0)
          _rewardToBeneficialVault(beneficialVaultBounty, _callerBalance, rewarderRewardPath);
        rewarderRewardToken.safeTransfer(_treasuryAccount, rewarderBounty.sub(beneficialVaultBounty));
      }
    }
    ...
    if (reinvestRewarderReward) {
      router.swapExactTokensForTokens(
        rewarderReward.sub(rewarderBounty),
        0,
        getRewarderReinvestPath(),
        address(this),
        now
      );
    }
  ...
  function setRewarderReinvestConfig(
    ...
    require(_reinvestBountyBps <= rewarderMaxReinvestBountyBps, "exceeded maxReinvestBountyBps.");
    require(_reinvestPath.length >= 2, "_reinvestPath length must >= 2.");
    require(
      _reinvestPath[0] == rewarderRewardToken && _reinvestPath[_reinvestPath.length - 1] == baseToken,
      "bad _reinvestPath."
    );
  ...
  function setRewarderMaxReinvestBountyBps(uint256 _maxReinvestBountyBps) external onlyOwner {
    require(_maxReinvestBountyBps >= rewarderReinvestBountyBps, "lower than reinvestBountyBps.");
    require(_maxReinvestBountyBps <= 3000, "exceeded 30%.");
  ...
  function setRewarderRewardPath(address[] calldata _rewardPath) external onlyOwner {
    require(_rewardPath.length >= 2, "_rewardPath length must be >= 2.");
    require(
      _rewardPath[0] == rewarderRewardToken && _rewardPath[_rewardPath.length - 1] == beneficialVault.token(),
      "bad _rewardPath."
    );
  ...
  function setRewarderTreasuryConfig(address _treasuryAccount, uint256 _treasuryBountyBps) external onlyOwner {
    require(_treasuryAccount != address(0), "bad _treasuryAccount.");
    require(_treasuryBountyBps <= rewarderMaxReinvestBountyBps, "exceeded maxReinvestBountyBps.");
