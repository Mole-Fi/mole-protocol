...
/// @title TraderJoeWorkerV2 is a worker with reinvest-optimized and beneficial vault buyback functionalities
contract DeprecatedTraderJoeWorkerV2 is OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe, IWorker03 {
  /// @notice Libraries
  ...
  IMasterChefJoeV2 public masterChefJoeV2;
  ...
  // Support rewarder(in TradeJoeMasterChef) reward token
  address public rewarderRewardToken;
  uint256 public rewarderRewardAmount;
  uint256 public rewarderRewardNativeAmount;
  address[] public rewarderRewardReinvestPath;
  address[] public rewarderRewardNativeReinvestPath;
  // address[] public rewarderRewardPath;

  function initialize(
    address _operator,
    address _baseToken,
    IMasterChefJoeV2 _masterChefJoeV2,
    ...
    masterChefJoeV2 = _masterChefJoeV2;
    router = _router;
    ...
    pid = _pid;
    (IERC20 _lpToken, , , , IRewarder _rewarder) = masterChefJoeV2.poolInfo(_pid);
    if (address(_rewarder) != address(0)) {
      IERC20 _rewardToken = _rewarder.rewardToken();
      rewarderRewardToken = address(_rewardToken);
    } else {
      rewarderRewardToken = address(0);
    }
    rewarderRewardAmount = 0;
    rewarderRewardNativeAmount = 0;

    lpToken = ISwapPairLike(address(_lpToken));
    address token0 = lpToken.token0();
    address token1 = lpToken.token1();
    farmingToken = token0 == baseToken ? token1 : token0;
    joe = address(masterChefJoeV2.joe());
    ...
    require(baseToken != joe, "baseToken must !joe");
    ...
    require(reinvestPath[0] == joe && reinvestPath[reinvestPath.length - 1] == baseToken, "bad reinvest path");
  }
  ...
  function shareToBalance(uint256 share) public view returns (uint256) {
    ...
    (uint256 totalBalance, ) = masterChefJoeV2.userInfo(pid, address(this));
  ...
  function _reinvest(
    ...
    // 1. Withdraw all the rewards. Return if reward token NO-CHANGE and <= _reinvestThreshold
    (bool changed, address newToken, uint256 newTokenAmount) = masterChefWithdrawInternal(pid, 0);

    uint256 reward = joe.balanceOf(address(this));
    //always reinvest if TokenChanged
    if (!changed) {
      //FIXME: 1、reinvest threshold when none rewarder, need to add new reinvestThreshold
      //FIXME: 2、if need reinvest new reward token
      // origin is reward <= _reinvestThreshold change <= to < 
      if (
        reward <= _reinvestThreshold ||
        (rewarderRewardAmount <= _reinvestThreshold && rewarderRewardNativeAmount <= _reinvestThreshold)
      ) return;
    }

    joe.safeApprove(address(router), uint256(-1));
    if (rewarderRewardToken != address(0)) {
      rewarderRewardToken.safeApprove(address(router), uint256(-1));
    }
    address(lpToken).safeApprove(address(masterChefJoeV2), uint256(-1));
    ...
    if (bounty > 0) {
      ...
      joe.safeTransfer(_treasuryAccount, bounty.sub(beneficialVaultBounty));
    }
    //FIXME: rewarder reward bounty to beneficial vault
    ...
    rewarderRewardSwap(rewarderRewardAmount, rewarderRewardNativeAmount);
    ...
    masterChefJoeV2.deposit(pid, lpToken.balanceOf(address(this)));

    // 7. Reset approvals
    joe.safeApprove(address(router), 0);
    if (rewarderRewardToken != address(0)) {
      rewarderRewardToken.safeApprove(address(router), uint256(0));
    }
    address(lpToken).safeApprove(address(masterChefJoeV2), 0);

    // 8. Change reward token
    if (changed) {
      rewarderRewardToken = newToken;
      rewarderRewardAmount = newTokenAmount;
    } else {
      rewarderRewardAmount = 0;
    }
    rewarderRewardNativeAmount = 0;
  ...
  function work(
    ...
    uint256 balance = _removeShare(id);
    ...
    address(lpToken).safeTransfer(strat, balance);
  ...
  function masterChefWithdrawInternal(uint256 _pid, uint256 _amount)
    internal
    returns (
      bool rewardTokenChanged,
      address newRewardToken,
      uint256 newRewardAmount
    )
  {
    //1. If rewarder change
    (, , , , IRewarder rewarder) = masterChefJoeV2.poolInfo(_pid);
    if (address(rewarder) == address(0)) {
      if (rewarderRewardToken != address(0)) {
        rewardTokenChanged = true;
        newRewardToken = address(0);
        newRewardAmount = 0;
      } else {
        rewardTokenChanged = false;
        newRewardToken = address(0);
        newRewardAmount = 0;
      }
      masterChefJoeV2.withdraw(_pid, _amount);
      return (rewardTokenChanged, newRewardToken, newRewardAmount);
    }
    address rewardToken = address(rewarder.rewardToken());
    if (rewarderRewardToken != rewardToken) {
      rewardTokenChanged = true;
      newRewardToken = rewardToken;
    } else {
      rewardTokenChanged = false;
      newRewardToken = address(0);
      newRewardAmount = 0;
    ...
    //2. Get amounts before
    uint256 rewardTokenBefore = 0;
    if (rewarderRewardToken != address(0)) {
      rewardTokenBefore = rewarderRewardToken.balanceOf(address(this));
    }

    uint256 newRewardTokenBefore = 0;
    if (newRewardToken != address(0)) {
      newRewardTokenBefore = newRewardToken.balanceOf(address(this));
    }

    uint256 rewardNativeBefore = address(this).balance;
    ...
    if (rewarderRewardToken != address(0)) {
      ...
      //if withdraw lpToken, need sub amount
      if (rewarderRewardToken == address(lpToken)) {
        rewardAmount = newRewardToken.balanceOf(address(this)).sub(_amount).sub(rewardTokenBefore);
      } else {
        rewardAmount = newRewardToken.balanceOf(address(this)).sub(rewardTokenBefore);
      }
      rewarderRewardAmount = rewarderRewardAmount.add(rewardAmount);
    }

    if (newRewardToken != address(0)) {
      //if withdraw lpToken, need sub amount
      if (newRewardToken == address(lpToken)) {
        newRewardAmount = newRewardToken.balanceOf(address(this)).sub(_amount).sub(newRewardTokenBefore);
      } else {
        newRewardAmount = newRewardToken.balanceOf(address(this)).sub(newRewardTokenBefore);
      }
    }

    rewarderRewardNativeAmount = rewarderRewardNativeAmount.add(address(this).balance.sub(rewardNativeBefore));

    return (rewardTokenChanged, newRewardToken, newRewardAmount);
  }

  function rewarderRewardSwap(uint256 rewardAmount, uint256 rewardNativeAmount) internal {
    //swap reward to baseToken
    if (rewarderRewardToken != address(0) && rewarderRewardToken != baseToken && rewardAmount > 0) {
      uint256 tokenBalance = rewarderRewardToken.balanceOf(address(this));
      if (rewardAmount > tokenBalance) {
        router.swapExactTokensForTokens(tokenBalance, 0, getRewarderRewardReinvestPath(), address(this), now);
      } else {
        router.swapExactTokensForTokens(rewardAmount, 0, getRewarderRewardReinvestPath(), address(this), now);
      }
    }
    //swap reward native to baseToken
    if (rewardNativeAmount > 0) {
      uint256 nativeBalance = address(this).balance;
      if (rewardNativeAmount > nativeBalance) {
        IWAVAX(wNative).deposit{ value: nativeBalance }();
        if(baseToken != wNative){
          router.swapExactTokensForTokens(nativeBalance, 0, getRewarderRewardNativeReinvestPath(), address(this), now);
        }
      } else {
        IWAVAX(wNative).deposit{ value: rewardNativeAmount }();
        if(baseToken != wNative){
          router.swapExactTokensForTokens(rewardNativeAmount, 0, getRewarderRewardNativeReinvestPath(), address(this), now);
        }
      }
    }
  }
  ...
  function liquidate(uint256 id) external override onlyOperator nonReentrant {
    // 1. Check reward token change
    _reinvest(treasuryAccount, treasuryBountyBps, actualBaseTokenBalance(), uint256(-1));
    // 2. Convert the position back to LP tokens and use liquidate strategy.
    uint256 balance = _removeShare(id);
    lpToken.transfer(address(liqStrat), balance);
    liqStrat.execute(address(0), 0, abi.encode(0));
    // 3. Return all available BaseToken back to the operator.
  ...
  /// @param _beneficialVaultBounty - The amount of JOE to be swapped to BTOKEN & send back to the Vault.
  /// @param _callerBalance - The balance that is owned by the msg.sender within the execution scope.
  ...
  function actualBaseTokenBalance() internal view returns (uint256) {
    if(rewarderRewardToken == baseToken){
      return baseToken.myBalance().sub(rewarderRewardAmount).sub(buybackAmount);
    }
  ...
  function _addShare(uint256 id) internal {
    ...
    if (balance > 0) {
      ...
      masterChefJoeV2.deposit(pid, balance);
      // 4. Update shares
  ...
  function _removeShare(uint256 id) internal returns (uint256) {
    ...
    if (share > 0) {
      ...
      masterChefJoeV2.withdraw(pid, balance);
  ...
  function getRewarderRewardReinvestPath() public view returns (address[] memory) {
    if (rewarderRewardReinvestPath.length != 0) {
      address[] memory path = rewarderRewardReinvestPath;
      //replace dynamic
      path[0] = rewarderRewardToken;
      return path;
    }
    address[] memory path;
    if (baseToken == wNative) {
  ...
  function getRewarderRewardNativeReinvestPath() public view returns (address[] memory) {
    if (rewarderRewardNativeReinvestPath.length != 0) return rewarderRewardNativeReinvestPath;
    address[] memory path;
    if (baseToken == wNative) {
      revert("bad invoke or baseToken");
    } else {
      path = new address[](2);
  ...
  function setReinvestConfig(
    ...
    require(_reinvestPath[0] == joe && _reinvestPath[_reinvestPath.length - 1] == baseToken, "bad _reinvestPath");
  ...
  function setRewarderRewardReinvestConfig(address[] calldata _reinvestPath, address[] calldata _nativeReinvestPath)
    external
    onlyOwner
  {
    require(_reinvestPath[_reinvestPath.length - 1] == baseToken, "bad _reinvestPath");
    require(
      _nativeReinvestPath[0] == wNative && _nativeReinvestPath[_nativeReinvestPath.length - 1] == baseToken,
      "bad _nativeReinvestPath"
    );

    rewarderRewardReinvestPath = _reinvestPath;
    rewarderRewardNativeReinvestPath = _nativeReinvestPath;
  ...
  function setRewardPath(address[] calldata _rewardPath) external onlyOwner {
    ...
    require(_rewardPath[0] == joe && _rewardPath[_rewardPath.length - 1] == beneficialVault.token(), "bad _rewardPath");
  ...
  function setBeneficialVaultConfig(
    ...
    require(
      _rewardPath[0] == joe && _rewardPath[_rewardPath.length - 1] == _beneficialVault.token(),
      "bad _rewardPath"
    );
