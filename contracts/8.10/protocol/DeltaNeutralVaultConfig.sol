...
contract DeltaNeutralVaultConfig is IDeltaNeutralVaultConfig, OwnableUpgradeable {
  ...
  event LogSetMoleBountyConfig(address indexed _caller, address _moleReinvestTreasury, uint256 _moleBountyBps);
  event LogSetMoleBeneficiaryConfig(
    address indexed _caller,
    address _moleBeneficiary,
    uint256 _moleBeneficiaryBps
  );
  ...
  uint256 private constant MAX_MOLE_BOUNTY_BPS = 2500;
  uint256 private constant MAX_MOLE_BENEFICIARY_BPS = 6000;
  ...
  /// @notice MOLE token
  address public moleToken;
  /// @notice The router to be used for swaping MOLE to other assets
  ...
  /// @notice MOLE treausry
  address public moleReinvestFeeTreasury;
  /// @notice MOLE bounty percentage.
  uint256 public moleBountyBps;
  /// @notice MOLE beneficiary. This is the address that will receive portion of the bounty.
  address public moleBeneficiary;
  /// @notice MOLE beneficiary percentage.
  uint256 public moleBeneficiaryBps;
  ...
  /// @notice Set moleBountyBps.
  /// @dev Must only be called by owner.
  /// @param _newMoleReinvestFeeTreasury The new address to received MOLE reinvest fee
  /// @param _newMoleBountyBps Fee from reinvesting MOLE to positions.
  function setMoleBountyConfig(address _newMoleReinvestFeeTreasury, uint256 _newMoleBountyBps)
    external
    onlyOwner
  {
    if (_newMoleBountyBps > MAX_MOLE_BOUNTY_BPS) {
      revert DeltaNeutralVaultConfig_TooMuchBounty(_newMoleBountyBps);
    }

    moleReinvestFeeTreasury = _newMoleReinvestFeeTreasury;
    moleBountyBps = _newMoleBountyBps;

    emit LogSetMoleBountyConfig(msg.sender, moleReinvestFeeTreasury, moleBountyBps);
  }

  /// @notice Set moleBeneficiaryBps.
  /// @dev Must only be called by owner.
  /// @param _newMoleBeneficiary The new address to received MOLE reinvest fee
  /// @param _newMoleBeneficiaryBps Fee from reinvesting MOLE to positions.
  function setMoleBeneficiaryConfig(address _newMoleBeneficiary, uint256 _newMoleBeneficiaryBps)
    external
    onlyOwner
  {
    if (_newMoleBeneficiaryBps > MAX_MOLE_BENEFICIARY_BPS) {
      revert DeltaNeutralVaultConfig_TooMuchBounty(_newMoleBeneficiaryBps);
    }

    moleBeneficiary = _newMoleBeneficiary;
    moleBeneficiaryBps = _newMoleBeneficiaryBps;

    emit LogSetMoleBeneficiaryConfig(msg.sender, moleBeneficiary, moleBeneficiaryBps);
  }
  ...
  function setReinvestPath(address[] calldata _reinvestPath) external onlyOwner {
    ...
    if (_reinvestPath[0] != moleToken) revert DeltaNeutralVaultConfig_InvalidReinvestPath();
